        ...
        if notification_start_date <= today < payment_date:
            ...
            if notification.last_acknowledged_date != today:
               ...
               # Email content for bill payment reminder
                ...
                body = (
                    f"Dear {user.user_name},\r\n"  # Assuming username exists; adjust as needed
                    "\r\n"
                ...
                send_mail(
                    ...
                    [email],  # To email (list)
                    fail_silently=False,  # Raise exception if email fails (optional)
                )
...
def savinghead(request):
    savingheadData = tbl_savinghead.objects.all()
    if request.method == 'POST':
...
def budget(request):
    ...
    budgetData = tbl_budget.objects.all()
    if request.method == 'POST':
...
def savingbody(request,id):
    savingbodyData = tbl_savingbody.objects.all()
    if request.method == 'POST':
...
def income(request):
    ...
    incomeData = tbl_income.objects.all()
    if request.method == 'POST':
    ...
    if request.method == 'POST':
        ...
        if latest_budget:
            ...
            if total_expense_after >= (budget_amount * 0.8):
                ...
                context = {
                    "username": user.user_name,  # Adjust if your model uses a different field
                    "category": expensetype.expensetype_name,  # Assuming expensetype has a name field
...
def repayment(request):
    repaymentData = tbl_repayment.objects.all()
    if request.method == 'POST':
...
def repaymentnotification(request):
    repaymentnotificationData = tbl_repaymentnotification.objects.all()
    if request.method == 'POST':
...
def financial_reports(request):
    ...
    # 10. Savings Goal Progress (Gauge Chart)
